...
class Mancala:
    ...
    def __init__(self, name_1, name_2):
        ...
        print(" Player 1,", name_1 + ", moves first by entering a number for the space to move. ")
        print(" ")
        self._board_object.print_board(name_1, name_2)
        while True:
            if self._a_turn:
                player = name_1
            else:
            ...
            if player == name_1:
                ...
                if x == 'exit':  # exit game
                    ...
                    if self._board_object.check_side("B"):  # returns true if all spaces on their side of board empty
                        if self._board_object.is_empty(x):  # if space empty
                            continue
                        if self.move('B', x):
                            self._board_object.print_board(name_1, name_2)
    ...
    def move(self, A_or_B, space): #returns true if player turn again, false if next player turn
        """makes move takes 'A' or 'B' as arg to designate player and space to move. Returns true if player's turn again
        and false if it's other player's turn"""
        ...
        while True:
            while num_marbles != 0:
                if route_position == 12:
                    ...
                    key = route[route_position]
                    self.check_points(key, A_or_B) # updates player value self._points if necesssary
                    board[key] += 1
                    num_marbles -= 1
            if board[key] == 1: # it just landed in an empty space
                break
            else:
                if key != 'B' and key != 'A':
        ...
        if self.ends_in_goal(key): # if true,
            return True   # need to check for game over back in move function right after this returns
    ...
    def next_turn(self, player, space):
        ...
        if board[key] == 1 and key != "A" and key != 'B':  # num_marbles is zero and they ended in a space with zero marbles
            ...
            if player == "A":
                ...
                print("You landed in an empty space. ", self._playerA.get_player_name() + "'s turn!")
                self._a_turn = True
            return True
        else:
            return False
